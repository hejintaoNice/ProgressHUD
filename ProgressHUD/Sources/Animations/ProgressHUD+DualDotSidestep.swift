...
extension ProgressHUD {
	...
	func animationDualDotSidestep(_ view: UIView) {
        ...
        let radius = 10.0
        let startY = height / 2
        let startX = width / 3.0
        let lineWidth = 3.0

        let hollowCircle = createCircleLayer(CGPoint(x: startX, y: startY), radius, true, view, lineWidth, UIColor.init(red: 118/255.0, green: 220/255.0, blue: 237/255.0, alpha: 1))

		let filledCircle = createCircleLayer(CGPoint(x: width - startX, y: startY), radius, true, view, lineWidth, UIColor(red: 0.451, green: 0.31, blue: 0.996, alpha: 1))

		let animationGroup1 = moveAnimationGroup(width - startX * 2)
		hollowCircle.add(animationGroup1, forKey: "animation")

		let animationGroup2 = moveAnimationGroup(-(width - startX * 2))
    ...
    private func createCircleLayer(_ center: CGPoint, _ radius: CGFloat, _ filled: Bool, _ view: UIView, _ lineWidth: Double, _ color: UIColor) -> CAShapeLayer {
		let path = UIBezierPath(arcCenter: center, radius: radius, startAngle: 0, endAngle: 2 * .pi, clockwise: true)

		let layer = CAShapeLayer()
		layer.path = path.cgPath
		layer.strokeColor = filled ? UIColor.clear.cgColor : color.cgColor
		layer.fillColor = filled ? color.cgColor : UIColor.clear.cgColor
